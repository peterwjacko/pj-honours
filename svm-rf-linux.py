...
def dataScaling (transformType, X_train, X_test):
    if transformType == "MinMax":
        ...
        scaler = Normalizer() # TODO: not defined??
    else:
...
objectFeatures = pd.read_csv(r'/home/peter/pj-honours/Data/AllROI_FS_Merged.csv')
...
TimeNow = datetime.now() 
TimeNow = TimeNow.strftime('%d-%m-%H-%M')
summaryTable = summaryTable.to_csv(f'/home/peter/data/DataStorage/{activeModel}/SummaryTable_{TimeNow}.csv')    
# %%
featuresCombo = featureSet # if featureSet = featuresAll
